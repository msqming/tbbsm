...
env.roledefs = {
    'myserver': ['root@193.112.112.38'],
}
env.PROJECT_NAME = 'tbbsm'
env.SETTINGS_BASE = 'tbbsm/tbbsm/settings.py'
env.DEPLOY_PATH = '/tbbsm-env'
env.VENV_ACTIVATE = os.path.join(env.DEPLOY_PATH, 'bin', 'activate')
env.PYPI_HOST = '193.112.112.38'
env.PYPI_INDEX = 'http://193.112.112.38:18080/simple'
env.PROCESS_COUNT = 2
env.PORT_PREFIX = 909

env.hosts = ['193.112.112.38']
env.user = 'root'
env.password = 'ROOT123.com'
...
def deploy(version, profile):
    ...
    with prefix('source %s' % env.VENV_ACTIVATE):
        run('pip3 install %s -i %s --trusted-host %s' % (
            package_name,
            env.PYPI_INDEX,
            env.PYPI_HOST,
        ))
